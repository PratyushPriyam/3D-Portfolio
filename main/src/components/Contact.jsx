...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if(form.name === "" || form.email === "" || form.message === "") {
      ...
      alert('Please fill the entire form first')
      }
    else {
      setLoading(true)

      // PublicId:  EoQpigKiJLE2WSp4P
      // TemplateId: template_y3yjzsa
      // ServeceId: service_3sje53g
      ...
      emailjs.send(
        ...
        "template_y3yjzsa",
        {
          ...
          to_email: 'pratyushpriyam100@gmail.com',
      ...
      <motion.div className='flex-[0.75] bg-black-100 p-8 rounded-2xl' variants={slideIn('left', 'tween', 0.2, 1)}>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className='mt-12 flex flex-col gap-8'>
          ...
          <label className='flex flex-col'>
            ...
            <textarea rows="7" name='message' value={form.message} onChange={handleChange} placeholder="What's you want to say ?"
            className='py-4 bg-tertiary px-6 placeholder:text-secondary text-white rounded-lg outline-none border-none font-medium'/>
          </label>

          <button type='submit' className='py-3 px-8 bg-tertiary outline-none w-fit text-white font-bold shadow-md shadow-primary rounded-xl'>
